...
threshold12Calib = cth/100
  local testLogger = optim.Logger('./logs12/20net_'..ich..'channels_12ct_Th'..cth..'.log')
  detPred = {}
  detWin = {}
local IM12C={}
  ...
  for j=1,pos_num do -- in "images in dataset" do
    ...
    if EffNScales>1 then
     ...
     detWin[j]={}
     detWin[j]=detmsc
     IM12C[j]=normalize(im,stat12calib,ich)
    ...
    --print(frame:size(1),frame:size(2),frame:size(3))
      ...
      if th>0 then
        for i=1,EffNScales do
          ...
          tempdets=applyCalibNet(calib12,IM12C[j],tempdets,FilterSize12,threshold12Calib,ich) 
...
recall=tp/(tp+fn)
precision=tp/(tp+fp)
    testLogger:add{['% Threshold'] = threshold12Calib,
                   ['% True positive'] = tp,
                   ['% False positive'] = fp,
                   ['% False negative'] = fn,
                   ...
                   ['% Precision']=precision,
                   ['% Recall']=recall,
                   ['% F Score']=2/(1/recall+1/precision)} --survived windows because nms removes some
                   collectgarbage()
