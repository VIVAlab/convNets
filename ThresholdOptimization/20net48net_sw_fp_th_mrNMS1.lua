...
threshold48Calib = cth/100
  local testLogger = optim.Logger('./logs48/2048net_'..ich..'channels_48ct_calib2Th'..cth..'NMS1_th50.log')
  detPred = {}
  detWin = {}
local IM12C={}
local IM48={}
...
----------------------nms
      ...
      if nmsth1<1 then
        for i=1,EffNScales do
          mask=detWin[j][i][{{},5}]:ge(classth1) -- labeled positive by first net
          mask2=detWin[j][i][{{},5}]:lt(classth1) -- labeled negative by first net
          detsmc[i]=detWin[j][i]:clone()
          sm=torch.sum(mask)
          sm2=torch.sum(mask2)
          if sm>0 then -- if there are positives
          ...
          print('sm='..sm,'sm2='..sm2)
          print((#mask)[1],(#mask2)[1],(#detWin[j][i])[1])
