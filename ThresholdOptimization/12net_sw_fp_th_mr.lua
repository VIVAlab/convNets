...
function IoU(d,D) --compare ioU of d with D. d is a row vector and D are a row ordered list of bounding box coordinates
  ...
  local x1c=torch.clamp(D[{{},1}],d[1],d[3]) --x1'<=x1<=x2'
  local x2c=torch.clamp(D[{{},3}],d[1],d[3]) --x1'<=x2<=x2'
  local Xfit=(x1c:eq(D[{{},1}])+x2c:eq(D[{{},3}])):ge(1)  --logic OR. if X fits, Xfit=1 else Xfit=0
  local y1c=torch.clamp(D[{{},2}],d[2],d[4]) --y1'<=y1<=y2'
  local y2c=torch.clamp(D[{{},4}],d[2],d[4]) --y1'<=y2<=y2'
  local Yfit=(y1c:eq(D[{{},2}])+y2c:eq(D[{{},4}])):ge(1)  --logic OR. if X fits, Xfit=1 else Xfit=0

  local XYfit=torch.cmul(Xfit,Yfit)--Xfit:eq(Yfit)  --XYfit =(Xfit AND Yfit)
  --compute intersectionarea
  InoU[XYfit]=torch.cmul(x2c[XYfit]-x1c[XYfit]+1,y2c[XYfit]-y1c[XYfit]+1) 
...
function applyNet(model, iimage, detections, isize, threshold,ich)
    if(detections ~= nil) then
        ...
        --[[
        local nBatch = 1000    -- Out of memery if above 1000 images (maybe more)
        if iimVec:size()[1] > nBatch then
            parts = torch.floor(iimVec:size()[1] / nBatch)
            o = torch.exp(model:forward(iimVec[{{1, nBatch},{},{},{}}]:cuda())):double()         
            for i = 1, (parts - 1) do
                o = o:cat(torch.exp(model:forward(iimVec[{{(i*nBatch)+1, (i+1)*nBatch},{},{},{}}]:cuda())):double(), 1)
            end
            if iimVec:size()[1]%nBatch ~= 0 then
                o = o:cat(torch.exp(model:forward(iimVec[{{(parts*nBatch)+1, -1},{},{},{}}]:cuda()):double()), 1)
            end
        else
...
for cth = 0,Nth-1 do  --calibth nms
threshold12Calib = cth/(Nth-1) 
  local testLogger = optim.Logger('./logs12/12net_'..ich..'channels_12ct_calibTh'..cth..'.log')
  detPred = {}
  detWin = {}
for j=1,pos_num do -- in "images in dataset" do
    -- grab frame
    print(database_path..testImgs[j])
    ...
    print(#rawresults)
    print_r(scales)
    print('#MAXscales='..nscales,'fov='..FilterSize1[1],'minfsize='..min_size,'scratio='..scratio,'W='..frame:size(3),'W='..frame:size(2))
    pause()
    --]]
    rawresults=nil
    ...
    --================Calibration 12 Net===================
    if(useNMS==1) then
        for i=1,EffNScales do detmsc[i]=nms(detmsc[i],nmsth1) end
        print('nms done')
    end

    im12c=normalize(im,stat12calib,ich)
    for i=1,EffNScales do detmsc[i]=applyCalibNet(calib12,im12c,detmsc[i],FilterSize1,threshold12Calib,ich) end
    im12c=nil
    print('12calib done')
...
for th=0,100 do
    ...
    for j = 1, pos_num do--#testImgs do
            ...
            iou=IoU(gt[j],detWin[j][{{},{1,4}}])
            m,ii=torch.max(iou,1)
            
            ispos=iou:ge(iou_th)
            isneg=iou:lt(iou_th)
            
            labeledpos=detWin[j][{{},5}]:ge(classth)
            labeledneg=detWin[j][{{},5}]:lt(classth)
            ...
            --print(iou)
            --print(detWin[j][{{},5}])
            print('tp='..tp,'fp='..fp,'fn='..fn,'recall='..tp/(tp+fn),'precision='..tp/(tp+fp))
            --pause()
            windows=windows+detWin[j]:size(1)
            iou=torch.Tensor()
            ispos=torch.Tensor()
    ...
    testLogger:add{['% Threshold'] = th,
                   ['% True positive'] = tp,
                   ['% False positive'] = fp,
                   ['% False negative'] = fn,
                   ['% Survived windows']=windows,
                   ['% Precision']=tp/(tp+fp),
                   ['% Recall']=tp/(tp+fn),
                   ['% F Score']=1/(1+2*(tp/(fp+fn)))} --survived windows because nms removes some
end

end --calibthreshold loop end
