...
ich=1;
F3=importdata(sprintf('%s%d%s','12net_',ich,'channels_12ct_calib2Th35.log'));
...
ich=3;
F1=importdata(sprintf('%s%d%s','12net_',ich,'channels_12ct_calib2Th35.log'));
...
title('1 and 3-channel precision/recall curve no calib')
...
xlabel('recall'),ylabel('precision')
title('precision/recall curve no calib')
...
%}
figure
h1=plot(threshold,Fscore)
hold on
scatter(threshold,Fscore,'b')
xlabel('threshold'),ylabel('F-score')
h2=plot(threshold1,Fscore1,'r')
hold on
scatter(threshold1,Fscore1,'r')
legend([h1 h2],'gray','RGB')
ylim([0 1])
...
title('recall wrt threshold with no calib ')
legend([h1 h2],'gray','RGB')
ylim([0 1])
...
title('recall and survived windows no Calib')
