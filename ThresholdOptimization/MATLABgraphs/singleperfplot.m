...
ich=3;
F3=importdata(sprintf('%s%d%s','12net_',ich,'channels_12ct_calib2Th0.log'));
...
ich=3;
F1=importdata(sprintf('%s%d%s','12netBootstrap_',ich,'channels_12ct_calib2Th0.log'));
...
title('recall and survived windows for RGB')
...
legend([h1 h2],'plain-RGB','Bootstrap')
ylim([0 1])
...
%{
figure(6)
plot3(recall1,percSwind1,threshold1)
xlabel('recall'),ylabel('% window kept'),zlabel('threshold')
figure
plot(threshold1,tp1)
xlabel('th'),ylabel('tp')
%}