   ...
   while true do  -- batch loop --------------------------------------
	...
	--print('t-='..tNEG)
      ...
      for i = 1+batchPOS*(tPOS-1),batchPOS*(tPOS) do
         xPOS[idx] = randBSC(trainData.data[1][shufflePOS[i]]):clone()--image.scale(trainData.data[1][shufflePOS[i]],mdlHeight,mdlWidth)
         ytPOS[idx] = 1
      ...
      for i = 1+batchNEG*(tNEG-1),batchNEG*(tNEG) do
         xNEG[idx] = randBSC(trainData.data[2][shuffleNEG[i]],ich,mdlHeight,mdlWidth):clone() ------------------randcropscale(img,ich,mdlH,mdlW)
         ytNEG[idx] = 2
         if torch.rand(1)[1]>.5 then
