...
thirdFilter = 24
forthFilter = 48
threshold48Calib = 0.06
scales = {0.6,0.4,0.3,0.25,0.2,0.16,0.13,0.1,0.08,0.07,0.06,0.05,0.04}
--scales = {0.1,0.08,0.05}
imgcnt = 1
...
network1 = torch.load('model3Cascade/model12-net-p.net')
net12 = nn.Sequential()
for i=1,5 do
...
c_tmp = torch.load('model3Cascade/model12-calib-p.net')
calib12 = nn.Sequential()
calib12:add(network1.modules[1])
calib12:add(network1.modules[2])
calib12:add(network1.modules[3])
calib12:add(c_tmp)
calib12:float()
...
function nms(boxes, overlap, si)
  ...
  while I:numel()>0 do 
    ...
    a=xx1/area[i]
    I = I[o:le(overlap) or a:ge(1 + si) or a:le(a - si)]
  end
...
function process()
   ...
   threshold12Calib = widget.t4.value/20000
   ...
   if(useNMS==1) then
		keep1nms = nms(detections1,0.6,0.1)
	else
   ...
   print("net24 done")
   --================Calibration 24 Net===================
   detections2 = applyCalibNetCropped(calib24,imVec,detectionsT,out2,thirdFilter,threshold24Calib,useNMS,0.6,0)
   ...
   detections3 = applyCalibNetCropped(calib48,imVec,detectionsT,out3,forthFilter,threshold48Calib,useGNMS,0.15,1) 
